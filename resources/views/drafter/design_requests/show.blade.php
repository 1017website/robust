...
<div class="drafter-ui">
    ...
    <form method="POST" action="{{ route('drafter.design-requests.feedback',$designRequest) }}" class="drafter-workspace">
        ...
        <main class="center-feedback" id="feedback">
            ...
            <div class="info-card mt-3" id="documents"><div class="card-head"><h2>6. Upload Drawing & Dokumen</h2></div><div class="doc-chip-row">@forelse($designRequest->documents->take(4) as $doc)<div class="doc-chip"><i class="bi bi-file-earmark-pdf"></i><span>{{ $doc->name }}<small>{{ $doc->humanSize() }}</small></span><a href="{{ asset('storage/'.$doc->file_path) }}" target="_blank" class="btn btn-sm btn-link" aria-label="Download dokumen"><i class="bi bi-download"></i></a></div>@empty<div class="upload-box small"><i class="bi bi-cloud-upload fs-3"></i><br>Upload drawing, render, shop drawing, atau BOQ dari menu Documents.</div>@endforelse</div></div>
