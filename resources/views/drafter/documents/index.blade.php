...
<div class="drafter-ui">
    ...
    <div class="drafter-shell">
        ...
        <aside class="drafter-detail">
            @if($selectedDocument)
                <div class="detail-top"><div><h2>{{ $selectedDocument->name }}</h2><div class="text-muted-2">{{ $selectedDocument->documentable?->code ?? 'Dokumen' }} / {{ \Illuminate\Support\Str::headline($selectedDocument->category ?: 'Lainnya') }}</div></div><a href="{{ route('documents.index') }}" class="btn btn-link" aria-label="Tutup detail"><i class="bi bi-x-lg"></i></a></div>
