...
<div class="drafter-ui">
    <div class="drafter-page-head">
        ...
        <div class="page-actions"><input class="form-control" style="width:220px" value="Bulan Ini ({{ now()->translatedFormat('F Y') }})" readonly><select class="form-select" style="width:170px"><option>Semua PIC</option></select><select class="form-select" style="width:170px"><option>Semua Project</option></select><button class="btn btn-soft"><i class="bi bi-download me-1"></i>Export</button></div>
    </div>
    <div class="drafter-stat-grid six">
        <div class="drafter-stat"><div class="ico blue"><i class="bi bi-folder"></i></div><div><div class="label">Total Project Aktif</div><div class="value">{{ $summary['active_projects'] }}</div><div class="sub up">↗ 2 dari bulan lalu</div></div></div>
        <div class="drafter-stat"><div class="ico green"><i class="bi bi-clipboard-check"></i></div><div><div class="label">Project Selesai</div><div class="value">{{ $summary['completed_projects'] }}</div><div class="sub up">↗ 3 dari bulan lalu</div></div></div>
        <div class="drafter-stat"><div class="ico purple"><i class="bi bi-ui-checks"></i></div><div><div class="label">Task Selesai</div><div class="value">{{ $summary['completed_tasks'] }}</div><div class="sub up">↗ 18 dari bulan lalu</div></div></div>
        <div class="drafter-stat"><div class="ico orange"><i class="bi bi-arrow-repeat"></i></div><div><div class="label">Revisi</div><div class="value">{{ $summary['revisi'] }}</div><div class="sub down">↓ 5 dari bulan lalu</div></div></div>
        <div class="drafter-stat"><div class="ico red"><i class="bi bi-exclamation-triangle"></i></div><div><div class="label">Overdue Task</div><div class="value">{{ $summary['overdue'] }}</div><div class="sub down">↓ 2 dari bulan lalu</div></div></div>
        <div class="drafter-stat"><div class="ico teal"><i class="bi bi-bullseye"></i></div><div><div class="label">On-Time Completion</div><div class="value">{{ $summary['on_time'] }}%</div><div class="sub up">↗ 5% dari bulan lalu</div></div></div>
    </div>
    ...
    <div class="drafter-report-grid">
        ...
        <div class="card-r"><div class="card-head"><h2>Aktivitas per Kategori</h2></div><div class="category-tile-grid"><div><i class="bi bi-calendar-event"></i><strong>Meeting</strong><span>12</span></div><div><i class="bi bi-buildings"></i><strong>Produksi</strong><span>{{ $statusSummary['drafting'] ?? 0 }}</span></div><div><i class="bi bi-check2-circle"></i><strong>QC</strong><span>{{ $statusSummary['review'] ?? 0 }}</span></div><div><i class="bi bi-file-earmark"></i><strong>Dokumen</strong><span>{{ $summary['completed_tasks'] }}</span></div></div></div>
    </div>
