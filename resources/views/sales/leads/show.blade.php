...
<div class="sales-ui">
    ...
    <div class="row g-3">
        <div class="col-xl-8">
            ...
            <div class="info-card mb-3"><h6><i class="bi bi-clipboard-check sgreen rounded p-2 me-2"></i>Kebutuhan Awal</h6><div class="row"><div class="col-md-6"><div class="kv"><div class="k">Nama Laboratorium / Proyek</div><div class="v">{{ $lead->lab_name ?: '-' }}</div></div><div class="kv"><div class="k">Deskripsi Kebutuhan</div><div class="v">{{ $lead->need_description ?: '-' }}</div></div></div><div class="col-md-6"><div class="k small text-muted-2 mb-2">Ruang Lingkup</div>@foreach(($lead->scope_items ?? []) as $item)<span class="tag-pill">{{ $item }}</span>@endforeach @if(empty($lead->scope_items))<span class="text-muted-2">-</span>@endif<div class="kv mt-2"><div class="k">Estimasi Budget</div><div class="v">{{ \App\Support\Format::rupiah($lead->est_value_min) }} - {{ \App\Support\Format::rupiah($lead->est_value_max) }}</div></div></div></div></div>
        ...
        <div class="col-xl-4">
            ...
            @if(auth()->user()->isSales())<div class="info-card"><h6>Tindak Lanjut Terjadwal</h6><a href="{{ route('activities.create') }}" class="btn btn-soft w-100">Buat Jadwal / Aktivitas</a></div>@endif
        </div>
    </div>
</div>
@endsection
