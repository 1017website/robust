    ...
    <div class="sales-page-head align-items-center">
        <div>
            ...
            <div class="card-r p-0 overflow-hidden">
                <form class="sales-filter-row p-3 pb-0" method="GET"><select name="status" class="form-select"><option value="">Semua Status</option>@foreach(\App\Models\DesignRequest::statuses() as $k=>$v)<option value="{{ $k }}" @selected(request('status')==$k)>{{ $v }}</option>@endforeach</select>@if(!auth()->user()->isSales())<select name="sales_id" class="form-select"><option value="">Semua Sales</option>@foreach($salesList as $sales)<option value="{{ $sales->id }}" @selected((string)request('sales_id')===(string)$sales->id)>{{ $sales->name }}</option>@endforeach</select>@endif<select name="production_pic_id" class="form-select"><option value="">Semua Produksi PIC</option>@foreach($drafters as $drafter)<option value="{{ $drafter->id }}" @selected((string)request('production_pic_id')===(string)$drafter->id)>{{ $drafter->name }}</option>@endforeach</select><div class="sales-search"><i class="bi bi-search"></i><input name="q" value="{{ request('q') }}" class="form-control" placeholder="Cari design request..."></div><button type="submit" class="btn btn-soft" aria-label="Filter"><i class="bi bi-funnel"></i></button></form>
            ...
            @if($selected)
                ...
                <div class="sales-detail-body design-request-detail-body">
                    ...
                    <div class="design-request-detail-sections">
                        ...
                        <div class="info-card"><h6>Assignment Produksi</h6><div class="d-flex gap-2 align-items-center">@if($selected->productionPic)<div class="mini-avatar">{{ strtoupper(substr($selected->productionPic->name,0,1)) }}</div><div><strong>{{ $selected->productionPic->name }}</strong><div class="small text-muted-2">Deadline {{ $selected->deadline?->translatedFormat('d M Y') }}</div></div>@else<span class="text-muted-2">Belum ditugaskan</span>@endif</div></div>
