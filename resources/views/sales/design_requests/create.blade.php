...
@section('content')
@php
    $isDraft = (bool) $designRequest;
    $val = fn (string $field, $fallback = null) => old($field, $designRequest?->{$field} ?? $fallback);
    $selectedScopes = old('scope_checklist', $designRequest?->scope_checklist ?? $lead?->scope_items ?? []);
        ...
        <div class="row g-3">
            <div class="col-xl-8">
                <div class="sales-form-card">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <input name="code" value="{{ $val('code') }}" class="form-control" maxlength="50" placeholder="Contoh: DR-2026-014 — kosongkan untuk nomor otomatis">
                        ...
                        <div class="col-md-4"><label class="form-label small fw-bold">Customer / Instansi *</label><input id="customer_name" name="customer_name" value="{{ $val('customer_name', $lead?->instansi) }}" class="form-control" required></div>
                        <div class="col-md-4"><label class="form-label small fw-bold">PIC Customer *</label><input id="pic_name" name="pic_name" value="{{ $val('pic_name', $lead?->pic_name) }}" class="form-control" required></div>
                        <div class="col-md-4"><label class="form-label small fw-bold">Nama Proyek *</label><input id="project_name" name="project_name" value="{{ $val('project_name', $lead?->lab_name) }}" class="form-control" required></div>
                        ...
                        <div class="col-md-3"><label class="form-label small fw-bold">Tanggal Request *</label><input type="date" name="request_date" value="{{ old('request_date', $designRequest?->request_date?->format('Y-m-d') ?? date('Y-m-d')) }}" class="form-control" required></div>
                        <div class="col-md-3"><label class="form-label small fw-bold">Deadline *</label><input type="date" name="deadline" value="{{ old('deadline', $designRequest?->deadline?->format('Y-m-d')) }}" class="form-control" required></div>
                        <div class="col-md-3"><label class="form-label small fw-bold">Status Urgensi *</label><select name="priority" class="form-select" required>@foreach(\App\Models\DesignRequest::urgencyOptions() as $k=>$v)<option value="{{ $k }}" @selected(old('priority', $designRequest?->priority ?? $defaultUrgency) === $k)>{{ $v }}</option>@endforeach</select></div>
                        <div class="col-12"><label class="form-label small fw-bold">Deskripsi Singkat *</label><textarea id="short_description" name="short_description" rows="3" maxlength="500" class="form-control" required>{{ $val('short_description', $lead?->need_description) }}</textarea></div>
                    </div>
                </div>
                ...
                <div class="sales-form-card">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4"><label class="form-label small fw-bold">Jenis Laboratorium / Area *</label><input id="lab_type" name="lab_type" value="{{ $val('lab_type', $lead?->lab_name) }}" class="form-control" required></div>
                        <div class="col-md-8">
                        ...
                        <div class="col-md-4"><label class="form-label small fw-bold">Kapasitas / Pengguna</label><input id="capacity" name="capacity" value="{{ $val('capacity', $lead?->capacity) }}" class="form-control"></div>
                        <div class="col-md-8"><label class="form-label small fw-bold">Detail Kebutuhan *</label><textarea id="detail_need" name="detail_need" rows="4" maxlength="1000" class="form-control" required>{{ $val('detail_need', $lead?->need_description) }}</textarea></div>
                    </div>
                </div>
                ...
                <div class="sales-form-card">
                    ...
                    <div class="mt-3"><label class="form-label small fw-bold">Catatan Tambahan</label><textarea name="extra_note" rows="3" class="form-control" maxlength="500">{{ $val('extra_note') }}</textarea></div>
                </div>
            </div>

            <div class="col-xl-4">
                @unless(auth()->user()->isSales())
                    <div class="sales-form-card"><h2 class="sales-form-title">Sales Owner</h2><select name="sales_id" class="form-select" required><option value="">Pilih sales</option>@foreach($salesList as $sales)<option value="{{ $sales->id }}" @selected((string) old('sales_id', $designRequest?->sales_id ?? $lead?->sales_id) === (string) $sales->id)>{{ $sales->name }}</option>@endforeach</select></div>
                @endunless
                <div class="sales-form-card">
                    <h2 class="sales-form-title">Assignment Drafter</h2>
                    <label class="form-label small fw-bold">Drafter *</label>
                    <select id="production_pic_id" name="production_pic_id" class="form-select" required><option value="">Pilih Drafter</option>@foreach($drafters as $drafter)<option value="{{ $drafter->id }}" @selected((string) old('production_pic_id', $designRequest?->production_pic_id) === (string) $drafter->id)>{{ $drafter->name }}</option>@endforeach</select>
                    ...
                    <div class="d-grid gap-2">
                        @forelse($drafterWorkloads as $row)
                            <label class="info-card py-2 d-flex align-items-center justify-content-between gap-2">
                                <span><input type="radio" data-drafter-pick value="{{ $row['drafter']->id }}" @checked((string) old('production_pic_id', $designRequest?->production_pic_id) === (string) $row['drafter']->id)> <strong>{{ $row['drafter']->name }}</strong></span>
...
designRequestForm?.addEventListener('submit', event => {
    ...
    // Jangan pakai designRequestForm.action: tombol bernama "action" di dalam form
    // membayangi properti tersebut sehingga yang terbaca elemennya, bukan URL.
    const xhr = new XMLHttpRequest();
    xhr.open('POST', designRequestForm.getAttribute('action'), true);
