...
<div class="sales-admin-ui">
    <div class="sa-customer-grid">
            ...
            <section class="sa-card p-0 overflow-hidden">
                ...
                <div class="table-wrap">
                    <table class="sa-table">
                        <thead><tr><th>No</th><th>Customer</th><th>Area</th><th>Divisi</th><th>Jenis Customer</th><th>PIC Utama</th><th>Sales PIC</th><th>Total Project</th>@if($showPrices)<th>Total Nilai (Rp)</th>@endif<th>Last Project</th><th>Status</th><th>Aksi</th></tr></thead>
                        <tbody>
                        @forelse($customers as $cust)
                            <tr class="customer-select-row {{ $selected && $selected->id === $cust->id ? 'selected' : '' }}" data-customer-href="{{ route('sales.customers.index', array_merge(request()->except(['page','hide_detail']), ['customer'=>$cust->id])) }}#customer-detail" tabindex="0" role="link" aria-label="Tampilkan detail {{ $cust->name }}">
                                ...
                                @if($showPrices)<td>{{ \App\Support\Format::rupiahShort($cust->quotations()->sum('grand_total')) }}</td>@endif
                        ...
                        @empty
                            <tr><td colspan="{{ $showPrices ? 12 : 11 }}"><x-empty text="Belum ada customer." /></td></tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <aside class="sa-customer-side" id="customer-detail" data-selected-customer="{{ $selected?->id }}">
            @if($selected)
                <div class="sa-card">
                <div class="d-flex justify-content-between align-items-start"><div><h2 class="mb-2">{{ $selected->name }}</h2><span class="status-soft {{ $stageClass($selected->pipeline_stage) }}">{{ \App\Models\Customer::stages()[$selected->pipeline_stage] ?? $selected->pipeline_stage }}</span></div><a class="btn btn-sm btn-link text-dark" href="{{ route('sales.customers.index', array_merge(request()->except('page'), ['hide_detail'=>1])) }}" aria-label="Tutup detail"><i class="bi bi-x-lg"></i></a></div>
                    ...
                    <div class="customer-tab-panel active" data-customer-panel="overview">
                        ...
                        <div class="sa-info-card mt-3"><h6>Statistik</h6><div class="sa-mini-stat-grid"><div><strong>{{ $selected->projects->count() }}</strong><span>Total Project</span></div>@if($showPrices)<div><strong>{{ \App\Support\Format::rupiahShort($selected->quotations->sum('grand_total')) }}</strong><span>Total Nilai</span></div>@endif<div><strong>{{ $selected->quotations->count() }}</strong><span>Total Penawaran</span></div><div><strong>{{ $selected->probability }}%</strong><span>Win Rate</span></div></div></div>
                    ...
                    <div class="customer-tab-panel" data-customer-panel="quotations"><div class="sa-info-card"><h6>Daftar Penawaran</h6>@forelse($selected->quotations as $quotation)<div class="customer-panel-row"><span><strong>{{ $quotation->code }}</strong>@if($showPrices)<small>{{ \App\Support\Format::rupiahShort($quotation->grand_total) }}</small>@endif</span><x-status-badge :status="$quotation->status" /></div>@empty<x-empty text="Belum ada penawaran." />@endforelse</div></div>
...
@push('scripts')
<script>
    document.querySelectorAll('[data-customer-href]').forEach(function (row) {
        function openCustomerDetail(event) {
            if (event.target.closest('a, button, input, select, textarea, label')) return;
            window.location.assign(row.dataset.customerHref);
        }

        row.addEventListener('click', openCustomerDetail);
