    ...
    <div class="col-lg-8">
        ...
        <div class="card-r">
            ...
            <div class="table-wrap">
                <table class="table-r"><thead><tr><th>Termin</th><th>Persen</th>@if($showPrices)<th>Nilai</th>@endif<th>Jatuh Tempo</th><th>Status</th></tr></thead><tbody>
                @forelse($project->terms as $term)
                    <tr><td class="fw-semibold">{{ $term->name }}</td><td>{{ rtrim(rtrim(number_format($term->percentage, 2), '0'), '.') }}%</td>@if($showPrices)<td class="fw-num">{{ \App\Support\Format::rupiah($term->amount) }}</td>@endif<td>{{ $term->due_date?->format('d M Y') ?? '—' }}</td><td><x-status-badge :status="$term->status" /></td></tr>
                @empty
                    <tr><td colspan="{{ $showPrices ? 5 : 4 }}"><x-empty text="Belum ada termin." /></td></tr>
                @endforelse
                </tbody></table>
            </div>
        </div>
