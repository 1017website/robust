...
<div class="row g-3">
    ...
    <div class="col-lg-4"><div class="card-r"><div class="card-head"><h2>Informasi</h2></div><div class="mb-2"><small class="text-muted-2 d-block">Sales Pembuat</small><strong>{{ $quotation->sales?->name ?? '—' }}</strong></div><div class="mb-2"><small class="text-muted-2 d-block">Jenis Penawaran</small><strong>{{ $quotation->isUploaded() ? 'Upload file' : 'Dibuat di sistem' }}</strong></div><div class="mb-2"><small class="text-muted-2 d-block">Tanggal Penawaran</small><strong>{{ $quotation->quote_date?->format('d M Y') }}</strong></div><div><small class="text-muted-2 d-block">Berlaku Sampai</small><strong>{{ $quotation->valid_until?->format('d M Y') }}</strong></div></div></div>
</div>
@endsection
