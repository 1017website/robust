        ...
        <div class="col-lg-8">
            <div class="card-r">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6"><label class="form-label small fw-semibold">Customer</label><select name="customer_id" class="form-select select2"><option value="">—</option>@foreach($customers as $c)<option value="{{ $c->id }}" @selected((string)old('customer_id')===(string)$c->id)>{{ $c->name }} | Area: {{ $c->area ?: '-' }} | Divisi: {{ $c->division ?: '-' }}</option>@endforeach</select></div>
                    <div class="col-md-6"><label class="form-label small fw-semibold">Pipeline Stage <i class="bi bi-info-circle text-muted" title="Menentukan tahap pipeline customer setelah aktivitas disimpan."></i></label><select name="pipeline_stage" class="form-select"><option value="">Otomatis — gunakan stage customer saat ini</option>@foreach(\App\Models\Customer::stages() as $k=>$v)<option value="{{ $k }}" @selected(old('pipeline_stage')===$k)>{{ $v }}</option>@endforeach</select><div class="form-text">Jika memilih stage baru, pipeline Customer juga akan langsung diperbarui.</div></div>
