                ...
                <section class="sa-card">
                    <div class="sa-activity-filter">
                        ...
                        <form method="GET" class="d-flex flex-wrap gap-2 ms-auto">
                            ...
                            <select name="status" class="form-select form-select-sm"><option value="">Semua Status</option>@foreach(['scheduled','in_progress','completed','pending','cancelled'] as $s)<option value="{{ $s }}" @selected(request('status')===$s)>{{ ucfirst(str_replace('_',' ', $s)) }}</option>@endforeach</select>
