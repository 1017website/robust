...
<div class="sales-ui">
    ...
    <div class="calendar-page-grid">
        <div>
            <div class="sales-toolbar">
                <div class="sales-chip-row">
                    ...
                    <span class="sales-chip">{{ $first->translatedFormat('F Y') }}</span>
                </div>
                <form method="GET" class="page-actions">
                    <input type="hidden" name="month" value="{{ $month }}"><input type="hidden" name="year" value="{{ $year }}">
                    <select name="type" class="form-select"><option value="">Semua Tipe Aktivitas</option>@foreach(\App\Models\Activity::types() as $key=>$label)<option value="{{ $key }}" @selected(request('type')===$key)>{{ $label }}</option>@endforeach</select>
                    @if(!auth()->user()->isSales())<select name="sales_id" class="form-select"><option value="">Semua Sales</option>@foreach($salesUsers as $sales)<option value="{{ $sales->id }}" @selected((string)request('sales_id')===(string)$sales->id)>{{ $sales->name }}</option>@endforeach</select>@endif
            ...
            <div class="sales-calendar-grid">
                ...
                @for($day=1;$day<=$daysInMonth;$day++)
                    ...
                    <div class="sales-cal-cell">
                        <div class="sales-cal-day calendar-day-dot {{ $date === now()->format('Y-m-d') ? 'active' : '' }}">{{ $day }}</div>
        ...
        <aside>
            <div class="info-card mb-3"><h6>Kalender Mini</h6><div class="d-flex justify-content-between align-items-center mb-2"><strong>{{ $first->translatedFormat('F Y') }}</strong><div><a href="{{ route('calendar.index',['month'=>$prevMonth->month,'year'=>$prevMonth->year]) }}" class="btn btn-sm btn-soft"><i class="bi bi-chevron-left"></i></a><a href="{{ route('calendar.index',['month'=>$nextMonth->month,'year'=>$nextMonth->year]) }}" class="btn btn-sm btn-soft"><i class="bi bi-chevron-right"></i></a></div></div><div class="calendar-mini-grid">@foreach(['Sen','Sel','Rab','Kam','Jum','Sab','Min'] as $d)<strong>{{ $d }}</strong>@endforeach @for($i=0;$i<$startBlank;$i++)<span></span>@endfor @for($d=1;$d<=$daysInMonth;$d++)<a href="{{ route('calendar.index',['month'=>$month,'year'=>$year]) }}" class="calendar-day-dot {{ $d==now()->day && $month==now()->month && $year==now()->year ? 'active' : '' }}">{{ $d }}</a>@endfor</div></div>
